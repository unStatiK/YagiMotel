...
public class GameFinishRequest {
    ...
    public static GameFinishRequest convertFromTensoulMap(AppConfig config, ObjectMapper mapper, Map tensoulReplayInfo) {
        if (validateTensoulMap(tensoulReplayInfo)) {
            try {
                Map logMap = (Map) tensoulReplayInfo.get("log");
                return GameFinishRequest.builder()
    ...
    private static List<String> extractPlayersNicknames(List playerMapping) {
        return (List<String>) playerMapping.stream().map(mapping -> ((Map) mapping).get("nickname"))
